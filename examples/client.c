...
void twclient_on_map_change(TwClient *client, MsgMapChange *map_change) {
	printf("got map change: %s\n", map_change->name);
}

void twclient_on_chunk(TwClient *client, Chunk *chunk) {
...
void twclient_on_normal_packet(TwClient *client, DDNetPacket *packet) {
	for(int i = 0; i < packet->chunks.len; i++) {
		Chunk *chunk = &packet->chunks.data[i];
...
void twclient_on_network_data(TwClient *client, uint8_t *buf, size_t len) {
	Error err = ERR_NONE;
	DDNetPacket packet = decode_packet(buf, len, &err);
	if(err != ERR_NONE) {
