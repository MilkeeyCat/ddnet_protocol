...
void twclient_on_chunk(TwClient *client, DDNetChunk *chunk) {
	...
	case DDNET_MSG_KIND_SV_BROADCAST:
		twclient_on_broadcast(client, &chunk->payload.msg.broadcast);
		return;
	case DDNET_MSG_KIND_SV_MOTD:
		twclient_on_motd(client, &chunk->payload.msg.motd);
		return;
	...
	// case DDNET_MSG_KIND_READY_TO_ENTER:
	...
	case DDNET_MSG_KIND_SV_CHAT:
		printf("ignoring boring chunk kind=%d\n", chunk->payload.kind);
		return;
	};
