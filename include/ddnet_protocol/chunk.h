...
/// number and are reliable. If the peer does not acknowledge the sequence
/// number the chunk is resend with the resend flag set.
typedef enum {
...
///
/// @note @ref DDProtoChunkHeader.sequence field is only used by vital
/// chunks.
typedef struct {
	...
	/// Set only if @ref DDProtoChunkHeader.flags & @ref
	/// DDPROTO_CHUNK_FLAG_VITAL. Is the amount of vital chunks that were
	/// already sent. But the number flips back to 0 when @ref
	/// DDPROTO_MAX_SEQUENCE is reached.
	uint16_t sequence;
} DDProtoChunkHeader;
...
/// @attention This is not the message id that is sent over the network.
typedef enum {
...
/// @ref DDProtoMessageKind to know which one to use.
typedef union {
...
/// To access the net message struct check the @ref DDProtoMessage.kind and
/// access the @ref DDProtoMessage.msg union accordingly.
typedef struct {
...
/// Vital messages are reliable and should be resend if they are not
/// acknowledged by the peer. Not all messages are vital to improve performance
