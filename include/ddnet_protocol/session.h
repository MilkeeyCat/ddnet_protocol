...
/// @brief State of a ddnet network connection.
///
...
typedef struct {
	...
	///
	/// Tracking how many vital messages were sent. This value should be
	/// included in every messages chunk header. The first vital message sent
	/// should have sequence number 1 in its chunk header.
	uint16_t sequence;

	/// @brief Acknowledge sequence number of the other party.
	///
	/// This is the @ref DDProtoSession.ack of the other peer. This value should
	/// never be sent. It is being received from the packet header the peer sent
	/// to us. And we can use it to compare it to our @ref
	/// DDProtoSession.sequence to make sure no messages were lost.
	uint16_t peer_ack;
...
///
/// @param sequence sequence number of the incoming chunk.
/// @param ack expected sequence number.
///
/// @returns
/// true - if the sequence number is already known and the chunk should be
/// dropped. \n
/// false - if the sequence number is off and we need to request a resend of
/// lost chunks.
