...
static DDNetError decode_game_message(DDNetChunk *chunk, DDNetMessageId msg_id, DDNetUnpacker *unpacker) {
	...
	case DDNET_MSG_SV_CHAT:
		...
		msg->sv_chat.team = ddnet_unpacker_get_int(unpacker);
		msg->sv_chat.client_id = ddnet_unpacker_get_int(unpacker);
		msg->sv_chat.message = ddnet_unpacker_get_string(unpacker);
		break;
...
size_t ddnet_encode_message(DDNetChunk *chunk, uint8_t *buf, DDNetError *err) {
	...
	case DDNET_MSG_KIND_SV_CHAT:
		ddnet_packer_add_int(&packer, msg->sv_chat.team);
		ddnet_packer_add_int(&packer, msg->sv_chat.client_id);
		ddnet_packer_add_string(&packer, msg->sv_chat.message);
		break;
