...
TEST(Chunk, HeaderVital) {
	...
	size_t header_size = ddproto_decode_chunk_header(bytes, &header);

	EXPECT_EQ(header.flags, DDPROTO_CHUNK_FLAG_VITAL);
...
TEST(Chunk, HeaderNotVital) {
	...
	uint8_t *buf = &bytes[0];
	DDProtoChunkHeader header;
	size_t header_size = ddproto_decode_chunk_header(buf, &header);
...
TEST(Chunk, PackNonVitalHeader) {
	DDProtoChunkHeader header = {
		...
		.size = 2};
	uint8_t output[8];
	size_t bytes_written = ddproto_encode_chunk_header(&header, output);
...
TEST(Chunk, BigMessageId) {
	...
	DDProtoPacketHeader header = {
		.num_chunks = 1};
	Context ctx = {
		.chunks = (DDProtoChunk *)malloc(sizeof(DDProtoChunk) * header.num_chunks),
		.len = 0,
	};
	size_t size = ddproto_fetch_chunks(buf, sizeof(bytes), &header, on_chunk, &ctx, &err);
	EXPECT_EQ(err, DDPROTO_ERR_NONE);
	EXPECT_EQ(ctx.len, 1);

	EXPECT_EQ(ctx.chunks[0].header.flags, DDPROTO_CHUNK_FLAG_VITAL);
