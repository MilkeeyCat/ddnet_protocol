...
TEST(Packer, SingleByteInts) {
	...
	EXPECT_EQ(packer.err, DDPROTO_ERR_NONE);
...
TEST(Unpacker, SingleByteInts) {
	...
	EXPECT_EQ(unpacker.err, DDPROTO_ERR_NONE);
...
TEST(Unpacker, Strings) {
	...
	EXPECT_EQ(unpacker.err, DDPROTO_ERR_STR_UNEXPECTED_EOF);
}
...
TEST(Unpacker, Booleans) {
	...
	EXPECT_EQ(unpacker.err, DDPROTO_ERR_NONE);
	EXPECT_EQ(ddproto_unpacker_get_bool(&unpacker), false);
	EXPECT_EQ(unpacker.err, DDPROTO_ERR_INVALID_BOOL); // 2 is out of range for bool
	EXPECT_EQ(ddproto_unpacker_get_bool(&unpacker), false);
	EXPECT_EQ(unpacker.err, DDPROTO_ERR_END_OF_BUFFER); // 0xcc has extension bit set for int
}
