...
TEST(NormalPacket, HeaderOk) {
	...
	header.ack = 0;
	header.flags = DDPROTO_PACKET_FLAG_RESEND;
	header.num_chunks = 1;
	uint8_t bytes[DDPROTO_PACKET_HEADER_SIZE];
	DDProtoError err = ddproto_encode_packet_header(&header, bytes);
...
TEST(NormalPacket, HeaderAckOob) {
	DDProtoPacketHeader header;
	header.ack = DDPROTO_MAX_SEQUENCE + 2;
	header.flags = DDPROTO_PACKET_FLAG_RESEND;
	header.num_chunks = 1;
	uint8_t bytes[DDPROTO_PACKET_HEADER_SIZE] = {};
	DDProtoError err = ddproto_encode_packet_header(&header, bytes);
	EXPECT_EQ(err, DDPROTO_ERR_ACK_OUT_OF_BOUNDS);
...
TEST(NormalPacket, Info) {
	...
	EXPECT_EQ(packet.chunks.data[0].payload.kind, DDPROTO_MSG_KIND_INFO);
	EXPECT_EQ(packet.chunks.data[0].header.sequence, 3);
	DDProtoMsgInfo info = packet.chunks.data[0].payload.msg.info;
...
TEST(NormalPacket, PackInfo) {
	DDProtoPacket packet = {
		.kind = DDPROTO_PACKET_NORMAL,
		.header = {.num_chunks = 1, .token = 0x3de3948d}};
	packet.chunks.data = (DDProtoChunk *)malloc(sizeof(DDProtoChunk) * packet.header.num_chunks);
	packet.chunks.len = packet.header.num_chunks;

	packet.chunks.data[0].header.flags = DDPROTO_CHUNK_FLAG_VITAL;
	packet.chunks.data[0].header.sequence = 3;
	packet.chunks.data[0].payload.kind = DDPROTO_MSG_KIND_INFO;
	packet.chunks.data[0].payload.msg.info.password = "";
	packet.chunks.data[0].payload.msg.info.version = "short version";

	ddproto_fill_chunk_header(&packet.chunks.data[0]);
...
TEST(NormalPacket, PackInfoWithBuilder) {
	DDProtoMessage messages[] = {ddproto_build_msg_info("")};
	DDProtoSession session = {.sequence = 2, .token = 0x3de3948d};
	DDProtoPacket packet = {};
	DDProtoError err = ddproto_build_packet(&packet, messages, (sizeof(messages) / sizeof(messages[0])), &session);
	EXPECT_EQ(err, DDPROTO_ERR_NONE);
...
TEST(NormalPacket, StartInfoAndRconCmdAndDDProtoErrors) {
	uint8_t bytes[] = {
	...
	DDProtoPacket packet = ddproto_decode_packet(bytes, sizeof(bytes), nullptr);
	ddproto_free_packet(&packet);
}
...
TEST(NormalPacket, StartInfoAndRconCmd) {
	...
	EXPECT_EQ(packet.chunks.data[0].payload.kind, DDPROTO_MSG_KIND_CL_STARTINFO);
	DDProtoMsgClStartInfo start_info = packet.chunks.data[0].payload.msg.start_info;
	...
	EXPECT_EQ(packet.chunks.data[1].payload.kind, DDPROTO_MSG_KIND_RCON_CMD);
...
TEST(NormalPacket, UnknownFakeMessage) {
	...
	EXPECT_EQ(packet.chunks.data[0].header.flags, DDPROTO_CHUNK_FLAG_VITAL);
	EXPECT_EQ(packet.chunks.data[0].header.sequence, 3);
	EXPECT_EQ(packet.chunks.data[0].header.size, 6);

	uint8_t repack[2048];
	size_t repack_size = ddproto_encode_packet(&packet, repack, sizeof(repack), &err);
	EXPECT_EQ(err, DDPROTO_ERR_NONE);
