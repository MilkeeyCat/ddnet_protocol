...
TEST(SnapSinglePacket, DeltaWithRemovedKeys) {
	...
	// this is a delta to a previous snap we do not have so the values look a bit funny
	// it also contains 3 removed keys which is the key part of this test
	uint8_t bytes[] = {
	...
	// these values are all low because
	// they are delta values that need to be added to the
	// previous snap data to get the real values
