...
TEST(SnapSinglePacket, FirstFullSnap) {
	...
	// this is the first snap being sent so it is not a delta
	// it was on a ddnet server so it includes ddnet ex uuids
	uint8_t bytes[] = {
	...
	// There is no vital flag set so the chunk header did not include
	// a sequence number. So it is not actually set to 0 it is just unset.
